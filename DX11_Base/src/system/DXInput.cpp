...
*@return	true : 今押された
*/
bool DXInput::GetIsJustPressedKeyboard(KeyCode code)
{
	if (m_KeyboardState[m_iStateIndex][static_cast<uint16_t>(code)] & 0x80 &&
		((m_KeyboardState[1 - m_iStateIndex][static_cast<uint16_t>(code)] & 0x80) == 0))
	{
		return true;
	}
	return false;
...
*@return	true : 今離された
*/
bool DXInput::GetIsJustPulledKeyboard(KeyCode code)
{
	if (((m_KeyboardState[m_iStateIndex][static_cast<uint16_t>(code)] & 0x80) == 0) &&
		m_KeyboardState[1 - m_iStateIndex][static_cast<uint16_t>(code)] & 0x80)
	{
		return true;
	}
	return false;
...
*@return	true : 押されている
*/
bool DXInput::GetIsPressedKeyboard(KeyCode code)
{
	if (m_KeyboardState[m_iStateIndex][static_cast<uint16_t>(code)] & 0x80)
	{
		return true;
...
*@return	true : 離されている
*/
bool DXInput::GetIsFreeKeyboard(KeyCode code)
{
	if ((m_KeyboardState[m_iStateIndex][static_cast<uint16_t>(code)] & 0x80) == 0)
	{
		return true;
...
float DXInput::GetHorizontal(long iPadID)
{
	return m_JoypadState[iPadID][m_iStateIndex].lX / 1000.f;
}

float DXInput::GetVertical(long iPadID)
{
	return -m_JoypadState[iPadID][m_iStateIndex].lY / 1000.f;
}
